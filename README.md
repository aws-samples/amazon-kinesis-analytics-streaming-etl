...
The Java application has already been downloaded to an Amazon Elastic Compute Cloud (Amazon EC2) instance that was provisioned by AWS CloudFormation. You just need to connect to the instance and execute the JAR file to start ingesting events into the stream. Follow the link of the `ConnectWithSessionManager` output if the CloufFormation template to connect to the EC2 instance.
